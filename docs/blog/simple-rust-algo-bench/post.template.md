...
One function, timed the same way four times. Five participants build in one image as five distinct binaries — the last campaign's audit found {{B9_BINARIES}} of them, each with its own receipt and SHA-256 — and each runs the same kernel over the same fixtures on one dedicated host: PageRank in every campaign, and WCC, BFS and triangle counting in the wider ones. `neo4j-graph` is the [`neo4j-labs/graph`](https://github.com/neo4j-labs/graph) Rust library, {{NEO4J_LIBRARY}}, through its builder and `graph::prelude`; it accumulates and returns `f32`, stops by its own rule, and performs no work accounting. `icebug` is the Arrow update of NetworKit, in C++. `icecat` is the Rust rewrite of those kernels. `grustcat` is those kernels over Grust's model, projected to packed Arrow adjacency. `grust` is Grust's own kernels over a `GraphProjection`, the general kernel over a backend-neutral graph.
...
**Per sweep, on the one pair of rows that carries the same precision under the same stopping rule with neither charging work to a meter** — `grust-next@unchecked+f32` beside `neo4j-graph`, both formed inside the last campaign's own bundle — of the {{LFL_TOTAL}} cells, {{LFL_BELOW}} put Grust's sweep at or below the reference participant's. Inside dispersion, above by less than the margin on the cell, {{LFL_INSIDE}} of the {{LFL_TOTAL}}: {{LFL_INSIDE_CELLS}}. **The remaining {{LFL_OUTSIDE}} are above by more than the margin on that cell, and all of them are at one thread on the protocol-size fixtures: {{LFL_OUTSIDE_PCT}}.** Nothing in this campaign explains those three, and none of the commits in the stack was aimed at them.

**Against the release Grust shipped before this work**, on the counted default rather than the unchecked row and read inside the last campaign, the same kernel's sweep is {{ANCHOR_MIN}} to {{ANCHOR_MAX}} of v0.22.0's over {{ANCHOR_CELLS}} cells, with both sides returning the same bits. That is the reading that spans the whole stack, because it is Grust against Grust inside one campaign.

**Work accounting used to cost and now mostly does not.** Over the {{METER_PULL_CELLS}} pull-kernel cells the meter's cost per sweep runs {{METER_PULL_B9}} in B9 where the campaign before it read {{METER_PULL_B7}}. Above L3 it is within noise of no meter at all: {{METER_LARGE_B9}} at {{LARGE_SIZE}} nodes and {{METER_XLARGE_B9}} at {{XLARGE_SIZE}}, against B7's {{METER_LARGE_B7}} and {{METER_XLARGE_B7}}. At the protocol size and full width it is reduced and not gone: {{METER_PROTOCOL_FULL_B9}} where B7 read {{METER_PROTOCOL_FULL_B7}}.

**One result moved the wrong way.** On the push kernel the meter costs {{METER_PUSH_B9}} in B9 against {{METER_PUSH_B7}} in B7 over its {{METER_PUSH_CELLS}} cells, and on {{METER_PUSH_WORSE}} of them the B9 figure exceeds the B7 figure by more than both margins together. **Why it moved is not established here, and this post does not attribute it.**
...
**Per sweep, because the rows stop at different counts.** The three PageRank rows that appear side by side stop at three different iteration counts under the same tolerance: `neo4j-graph` between {{B9_NEO_MIN}} and {{B9_NEO_MAX}} sweeps, Grust at `f32` at {{B9_F32_MIN}} to {{B9_F32_MAX}}, Grust at `f64` at {{B9_F64_MIN}} to {{B9_F64_MAX}}. Two rows that did not take the same number of sweeps over the arcs did not do the same work, so **their totals are not comparable and their per-sweep times are.** A total is the time to reach the tolerance under that participant's own stopping rule, which is the number a reader who wants an answer should take; a per-iteration figure is the cost of one sweep and nothing else. Every distance drawn below is drawn on the per-sweep column, with the sum of the two cells' MAD over median as its margin, and a distance smaller than its margin is inside dispersion and is not a difference.

**No absolute crosses a campaign.** The host was restarted between the last two campaigns, and the participants whose code did not change moved anyway: over {{UNCHANGED_CELLS}} cells of unchanged code the median B9-over-B7 per-sweep ratio is {{UNCHANGED_MEDIAN}}. A millisecond in one campaign is not a millisecond in the next, so every comparison here is either between two cells of one campaign, or between a ratio formed inside one campaign and a ratio formed inside another. The method section prints the size of that drift, and the one place where a cell of one campaign meets a cell of another.
...
A cell's dispersion is its MAD over its median, and a cell at or above {{B9_DISPERSION_RULE}} is unusable and is not published. A distance between two cells carries the sum of their dispersions as its margin, and a distance smaller than its margin is not a difference.

Drift between campaigns is measured rather than assumed. Over {{UNCHANGED_CELLS}} cells of unchanged code the median B9-over-B7 per-sweep ratio is {{UNCHANGED_MEDIAN}}, with individual cells from {{UNCHANGED_MIN}} ({{UNCHANGED_MIN_CELL}}) to {{UNCHANGED_MAX}} ({{UNCHANGED_MAX_CELL}}); the reference participant's own one-thread sweep above L3 reads {{UNCHANGED_NEO_2M}}, on code that did not change. The one place a cell of one campaign meets a cell of another is {{ASSERT_NO_CROSS}}, and nowhere else; that table exists to show the size of what the rule is avoiding.

### The host, and the record

B9's timed campaign ran from {{B9_CAMPAIGN_START}} to {{B9_CAMPAIGN_END}}: {{B9_TIMED_RUNS}} runs, all clean, none discarded, none rerun, {{B9_PARITY_INVOCATIONS}} parity invocations before any of them, and the page cache dropped after parity and before the first timed run. {{B9_CELLS}} cells, {{B9_UNUSABLE}} of them at or above the threshold that makes a cell unusable; the largest dispersion was {{B9_WORST_DISPERSION}}, {{B9_WORST_CELL}}. Steal was at most {{B9_STEAL_MAX}} ticks over a run, a run with a sighting is discarded rather than published, and {{B9_RESIDENTS}} seen by name across the campaign. The record also says what it lost: the host script that writes the image receipt and the fixture manifest has a previous campaign's prefix hard-coded, so running it for B9 overwrote the previous campaign's receipt, which was read back from the image still on the host. That is in the bundle's own README rather than repaired quietly.
